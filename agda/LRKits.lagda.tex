...
\begin{code}
  module _ {𝓥 : Ctx → Ty → Set} where
    ...
    env-+ (ρ , sp) =
      let fit-hereₗ ↘, sp′ ,↙ fit-hereᵣ = ρ .Ψ .rel-+ₘ (sp , ρ .fit-here) in
    ...
    env-* (ρ , sp) =
      let fit-here′ , sp′ = ρ .Ψ .rel-*ₘ (sp , ρ .fit-here) in
...
\begin{code}
    ...
    bindEnv ρ .Ψ = [ [ ρ .Ψ │ 0ᴿ ]ᴿ ─ [ 0ᴿ │ 1ᴿ ]ᴿ ]ᴿ
    ...
    bindEnv ρ .lookup ((ul , ur) ↘, sp+ ,↙ (dl , dr)) (lvar (↙ i) q b)
      ...
      (ρ .lookup (ρ .Ψ .rel-≤ₘ ≤*-refl (+ₘ-identityʳ→ (sp+l , dl)) ul)
        (lvar i q bl))
    bindEnv ρ .lookup ((ul , ur) ↘, sp+ ,↙ (dl , dr)) (lvar (↘ i) q b)
      | sp+l , sp+r | bl , br = vr (lvar (↘ i) q
      (≤*-trans (+ₘ-identityʳ→ (sp+l , dl))
        (0*→≤* (ρ .Ψ .rel-0ₘ (≤*→0* bl , ul)))
      ++ₙ
      ≤*-trans (+ₘ-identityˡ→ (ur , sp+r)) (≤*-trans dr br)))
\end{code}
%</bindEnv>
