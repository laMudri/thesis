Modal logics, and in particular their modalities, are traditionally presented in
an axiomatic style.
...
necessitation rule \TirName{N} is a true restriction.
...
\begin{proposition}
  In NJ + \TirName{N} + \TirName{K} + \TirName{T}, the following principle is
  admissible.
  \[
    \ebrule{%
...
\begin{proof}
  By induction on $n$.
  When $n = 0$, \TirName{NKT} becomes just the necessitation rule \TirName{N}.
  For $n = \lvert\Gamma\rvert + 1$, we have the following derivation, where
  $\Box\Gamma$ abbrevates
  $\Box\Gamma_1, \ldots, \Box\Gamma_{\lvert\Gamma\rvert}$ and \TirName{T$'$} and
  \TirName{K$'$} abbrevate the obvious combinations of
  \TirName{T} and \TirName{K}, respectively, and \TirName{$\to$-E}.
...
\TirName{NKT} deriving $\Box A, \Box B \vdash \Box C$, it becomes unclear how to
conclude $\Box A \wedge \Box B \vdash \Box C$, as \TirName{NKT} does not apply.
...
adapted version of \TirName{NKT}.
...
\TirName{NKT} as the introduction rule for $\Box$.
