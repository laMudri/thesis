...
\begin{proof}
  ...
  \begin{mathpar}
    \ebrule{%
      \infer0[Var]{\Box\plr{A \to B} \vdash \Box\plr{A \to B}}
      \infer1[T$'$]{\Box\plr{A \to B} \vdash A \to B}
      \infer0[Var]{\Box A \vdash \Box A}
      \infer1[T$'$]{\Box A \vdash A}
...
style.
However, we do not want to add \TirName{Promotion} to our system because it is
...
\TirName{T$'$} is the same abbreviation as it was in the proof of
\cref{thm:PT-NKT4}.
...
$\Box\plr{B \to C} \wedge \Box B \vdash \Box C$ in NJ + \TirName{Promotion} +
\TirName{T}.
