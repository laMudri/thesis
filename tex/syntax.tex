...
The \AgdaDatatype{Premises} data type describes premises of rules,
...
This will be provided by the \AgdaRecord{Rule} associated to each label in a
\AgdaRecord{System}.
...
contexts.
An \emph{open type} (\AgdaFunction{OpenType}) is a type
indexed over a context.
These are the things that bunched connectives connect, and thus serve as the
interpretation of \AgdaDatatype{Premises}.

\ExecuteMetaData[\Syntaxtex]{OpenType}

An \emph{open family} (\AgdaFunction{OpenFam}) is
...
An \emph{extensible open family} (\AgdaFunction{ExtOpenFam})
...
\ExecuteMetaData[\Syntaxtex]{ExtOpenFam}

To specify the meaning of descriptions, let us assume some
extensible open family \AgdaBound{X},
...
annotation $\gr1$ (written \AgdaInductiveConstructor{1\#} in Agda) to make them
behave linearly.
...
Recall that \AgdaDatatype{Hand} is a type containing two inhabitants ---
\AgdaInductiveConstructor{ll} and \AgdaInductiveConstructor{rr}.
I use the notation \AgdaFunction{[} \AgdaBound{X} \AgdaFunction{>}
\AgdaBound{i} \AgdaFunction{<} \AgdaBound{Y} \AgdaFunction{]}, for
\AgdaBound{i} \AgdaSymbol: \AgdaDatatype{Hand}, to mean \AgdaBound{X} when
\AgdaBound{i} is \AgdaInductiveConstructor{ll} and \AgdaBound{Y} when
\AgdaBound{i} is \AgdaInductiveConstructor{rr}.
