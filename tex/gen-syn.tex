...
as \ExecuteMetaData[\SimpleSyntaxtex]{single-premise}.
I write the empty tree of premises as \AgdaInductiveConstructor{`$\dot1$}, and
conjoin trees of premises using \AgdaInductiveConstructor{\_`$\dottimes$\_}.
...
\ExecuteMetaData[\SimpleSyntaxtex]{Premises}
...
\AgdaInductiveConstructor{\_`$\rightarrowtriangle$\_}.
Given that this is the only form a rule can take, I define \AgdaRecord{Rule} as
a \AgdaKeyword{record} type rather than a \AgdaKeyword{data} type, so as to get
...
\ExecuteMetaData[\SimpleSyntaxtex]{Rule}
\ExecuteMetaData[\SimpleSyntaxtex]{semr}

Finally, a whole \AgdaRecord{System} comprises a set \AgdaField{Label} of rule
labels, and \AgdaField{rules}\AgdaSpace{}\AgdaSymbol{:}\AgdaSpace{}%
\AgdaField{Label}\AgdaSpace{}\AgdaSymbol{$\to$}\AgdaSpace{}\AgdaRecord{Rule}.
...
\ExecuteMetaData[\SimpleSyntaxtex]{System}
