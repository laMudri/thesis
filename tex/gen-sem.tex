...
\begin{figure}
  \begin{minipage}[t]{0.333\textwidth}
    \ExecuteMetaData[\SimpleSem]{dotone}
  \end{minipage}
  \begin{minipage}[t]{0.666\textwidth}
    \ExecuteMetaData[\SimpleSem]{dottimes}
  \end{minipage}
  \begin{minipage}[t]{0.549\textwidth}
    \ExecuteMetaData[\SimpleSem]{dotto}
  \end{minipage}
  \begin{minipage}[t]{0.450\textwidth}
    \ExecuteMetaData[\SimpleSem]{allb}
  \end{minipage}
  \begin{minipage}[t]{0.450\textwidth}
  ...
  \end{minipage}
  \caption{Simple connectives on \AgdaRecord{Ctx}\AgdaSpace{}\AgdaSymbol{$\to$}%
    \AgdaSpace{}\AgdaPrimitive{Set}}
...
In brief, \AgdaFunction{$\dot1$}, \AgdaFunction{\_$\dottimes$\_}, and
\AgdaFunction{\_$\dotto$\_} are pointwise liftings of the singleton set, the
Cartesian product of sets, and function sets from \AgdaPrimitive{Set} to
\AgdaRecord{Ctx}\AgdaSpace{}\AgdaSymbol{$\to$}\AgdaSpace{}\AgdaPrimitive{Set}.
...
same context.
I give the definition of each of the fields of \AgdaFunction{RenSem} using
copattern matching.
The definition below defines, for example, that taking the
\AgdaField{$\llbracket$var$\rrbracket$} projection of \AgdaFunction{RenSem} will
yield \AgdaInductiveConstructor{var}, and that taking the
\AgdaField{$\llbracket$var$\rrbracket$} projection and then passing an argument
\AgdaBound{b} will yield \AgdaInductiveConstructor{lam}\AgdaSpace{}%
\AgdaSymbol{(}\AgdaBound{b}\AgdaSpace{}\AgdaSymbol{(}%
\AgdaFunction{$\searrow^v$}\AgdaSpace{}\AgdaFunction{here$^v$}\AgdaSymbol{)}%
\AgdaSymbol{)}.
...
\ExecuteMetaData[\SimpleSem]{semTy}
\ExecuteMetaData[\SimpleSem]{semCtx}
\ExecuteMetaData[\SimpleSem]{semvdash}
...
\AgdaRecord{Ctx}\AgdaSpace{}\AgdaSymbol{$\to$}\AgdaSpace{}\AgdaDatatype{Ty}%
\AgdaSpace{}\AgdaSymbol{$\to$}\AgdaSpace{}\AgdaPrimitive{Set}
(for example, \AgdaRecord{\_$\ni$\_}, \AgdaDatatype{\_$\vdash$\_},
\AgdaBound{$\V$}, and \AgdaBound{$\C$}),
I introduce the abbreviation \AgdaFunction{OpenFam} for this type, and use the
...
specific datatype \AgdaDatatype{Ty} has been replaced by a module parameter
\AgdaBound{Ty}, hence the change in colour.

\ExecuteMetaData[\SimpleSyntaxtex]{OpenFam}
