...
We have instantiated the usage elaborator so that \AgdaField{0\#$^{-1}$},
\AgdaField{+$^{-1}$}, and \AgdaField{*$^{-1}$} are always singletons (i.e.,
deterministic), and \AgdaField{1\#$^{-1}$} is a singleton whenever it can be
(and empty for $\downarrow\downarrow$ and $\wn\wn$).
The notation \AgdaFunction{\#}\AgdaSpace{}\AgdaBound{i} turns statically known
natural number \AgdaBound{i} into the corresponding variable pointer, with
\AgdaNumber{0} being the leftmost.
In other words, we can write variables using de Bruijn levels.
The definition \AgdaFunction{\#0} is a workaround for an Agda bug.

\ExecuteMetaData[\Monotonicitytex]{minus}
