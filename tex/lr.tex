...
replaced by equality constraints, and the \TirName{Subuse} rule is turned into a
non-syntax-directed defining rule of the system.
Indeed, a similar system is presented by \citet{Granule18} among others.
I prefer \TirName{Subuse} to be admissible in this thesis because it means that
...
\TirName{Subuse} rule, though one could imagine adapting the operators there to
introduce tightening of usage constraints.
In terms of \cref{sec:framework}, where I consider $\name$-like syntaxes in
general, one would expect to add a \TirName{Subuse} rule, similarly to the
\TirName{Var} rule present in all syntaxes, if subusaging were not already
admissible.
